...
void ReceptionDlg::CreateControl()
{
	...
	else  // 학교 로고가 없는 경우
	{
		...
		m_txtCompanyName.Create(_T(""), WS_CHILD | WS_VISIBLE | SS_LEFT, CRect(20, 80, 197, 136), this, IDC_txtCompanyName);
...
void ReceptionDlg::drawBanner()
{
	...
	{
		bannerImg.Format(L"C:\\Program Files (x86)\\METASTORY\\CareCon\\%s", m_App->bannerImg[0]);
		if (m_App->bannerNum > 1)
			nBannerNo = 1;
	}
	else if (m_App->bannerNum >= 1 && nBannerNo == 1)
	{
		bannerImg.Format(L"C:\\Program Files (x86)\\METASTORY\\CareCon\\%s", m_App->bannerImg[1]);
	...
	{
		bannerImg.Format(L"C:\\Program Files (x86)\\METASTORY\\CareCon\\%s", m_App->bannerImg[2]);
		nBannerNo = 0;
	}
...
void ReceptionDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s", m_App->comp_idx, m_App->activeMacCode);
		m_App->linkURL(url);
	}
	...
	else if (m_rcReceiptSubject1.PtInRect(point))	
	{
		if (m_App->receiptSubject[0] != L"")
		{
			url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s&receipt_idx=%s&sub_type=viewform", 
				m_App->comp_idx, m_App->activeMacCode, m_App->receiptIdx[0]);
			m_App->linkURL(url);
		}
	}
	else if (m_rcReceiptSubject2.PtInRect(point))
	{
		...
		{
			url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s&receipt_idx=%s&sub_type=viewform",
				m_App->comp_idx, m_App->activeMacCode, m_App->receiptIdx[1]);
			m_App->linkURL(url);
		}
	}
	else if (m_rcReceiptSubject3.PtInRect(point))
	{
		...
		{
			url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s&receipt_idx=%s&sub_type=viewform",
				m_App->comp_idx, m_App->activeMacCode, m_App->receiptIdx[2]);
			m_App->linkURL(url);
		}
	}
	else if (m_rcReceiptSubject4.PtInRect(point))
	{
		...
		{
			url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s&receipt_idx=%s&sub_type=viewform",
				m_App->comp_idx, m_App->activeMacCode, m_App->receiptIdx[3]);
			m_App->linkURL(url);
		}
	}
	else if (m_rReset.PtInRect(point))
	{
		...
		{
			HKEY hKey;
			LONG result = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\METASTORY", 0, KEY_ALL_ACCESS, &hKey);
			if (result == ERROR_SUCCESS) {
				RegDeleteKey(hKey, _T("CareCon")); // 하위 키 삭제
				RegCloseKey(hKey);
			}
...
void ReceptionDlg::buttonURL(int ndx)
{
	...
	{
		url.Format(L"https://new.bizstory.co.kr/agent/cboard.php?comp_idx=%s&macaddress=%s", m_App->comp_idx, m_App->activeMacCode);
		m_App->linkURL(url);
	}
	...
	{
		url.Format(L"https://new.bizstory.co.kr/agent/bnotice.php?comp_idx=%s&macaddress=%s", m_App->comp_idx, m_App->activeMacCode);
		m_App->linkURL(url);
	}
	else if (m_App->buttonType[ndx] == L"4")
	{
		url.Format(L"https://new.bizstory.co.kr/agent/consult.php?comp_idx=%s&macaddress=%s", m_App->comp_idx, m_App->activeMacCode);
		m_App->linkURL(url);
	}
	else if (ndx == 0 && m_App->buttonName[0] == L"접수하기")
	{
		url.Format(L"https://new.bizstory.co.kr/agent/receipt.php?comp_idx=%s&macaddress=%s&sub_type=postform", m_App->comp_idx, m_App->activeMacCode);
		m_App->linkURL(url);
	}
