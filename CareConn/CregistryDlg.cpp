...
BOOL CregistryDlg::ConnectDB()
{
	...
	if (mysqlConn == NULL)
	{
		...
		{ 
			ShellExecute(nullptr, _T("open"), GetInstallDir() + L"CareConUpdate.exe", nullptr, nullptr, SW_SHOWNORMAL);
		}
...
void CregistryDlg::LoadCompanyInfo(char cls, CString companyName)
{
	...
	if (cls == '\0')
		strQuery.Format(L"SELECT sc_name, comp_name, tel_num, comp_code FROM %s.bizstory.company_info", m_App->dbName.GetString());
	else if (cls == '\0' && companyName != "")
		strQuery.Format(L"SELECT sc_name, comp_name, tel_num, comp_code FROM %s.company_info where comp_name like '%s'", m_App->dbName.GetString(), companyName.GetString());
	else
		strQuery.Format(L"SELECT sc_name, comp_name, tel_num, comp_code FROM %s.company_info where comp_class = '%c' and comp_name like '%s'", m_App->dbName.GetString(), cls, companyName.GetString());
...
BOOL CregistryDlg::CheckMacAddr()
{
	...
	strQuery.Format(L"SELECT macaddress1, macaddress2 FROM %s.carecon_info where carecon_comp_code='%s'", m_App->dbName.GetString(), m_App->carecon_compCode.GetString());
	...
	if (resultMsg != 0)
	{
		AfxMessageBox(L"Database에서 CareConn(carecon_info)를 가져오지 못했습니다");
		LogWrite(L"MySQL Error", L"Database에서 CareConn 정보(carecon_info)를 가져오지 못했습니다");
		return false;
	}
	...
	{
		AfxMessageBox(L"Database에서 CareConn(carecon_info)를 가져오지 못했습니다");
		LogWrite(L"MySQL Error", L"Database Error : carecon_info mysql_store_result");
		return false;
	}
...
void CregistryDlg::OnBnClickedOk()
{
	...
	if (nResp == IDYES)
	{
		...
		strQuery.Format(L"SELECT carecon_comp_code FROM %s.carecon_info where comp_code='%s' and (macaddress1='%s' or macaddress2='%s')",
			m_App->dbName.GetString(), comp_code.GetString(), m_App->activeMacCode.GetString(), m_App->activeMacCode.GetString());
		strquery = W2A(strQuery);
		resultMsg = mysql_query(&conn, strquery);
		...
		if (sql_row != NULL)
		{
			...
			// update del_yn -> y 
			//strQuery.Format(L"UPDATE %s.carecon_info SET del_yn = 'Y', ver_info = '1.1.4' WHERE macaddress1='%s' or macaddress2='%s';",
			//	m_App->dbName, m_App->activeMacCode, m_App->activeMacCode);

			strQuery.Format(L"UPDATE %s.carecon_info SET del_yn = 'Y', ver_info = '%s' WHERE macaddress1='%s' or macaddress2='%s';",
				m_App->dbName.GetString(), m_App->strVersion.GetString(), m_App->activeMacCode.GetString(), m_App->activeMacCode.GetString());
		...
		m_App->carecon_compCode.Format(L"%s-%04d%02d%02d%02d%02d%02d", comp_code.GetString(),
		...
		// CareCon_Version
		strQuery1.Format(L"INSERT INTO %s.carecon_info(comp_code, ver_info, carecon_comp_code, macaddress1, reg_date, last_date)", m_App->dbName.GetString()); // , user, install_place)", m_App->dbName);
		strQuery2.Format(L" VALUES('%s','%s','%s','%s','%s','%s');", //, '%s', '%s');",
			comp_code.GetString(), m_App->strVersion.GetString(), m_App->carecon_compCode.GetString(), m_App->activeMacCode.GetString(), regDate.GetString(), lastDate.GetString());// , mUser, mPlace);
		...
		SetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"carecon_comp_code", m_App->carecon_compCode);
		...
		{
			//CString tst;
			//tst.Format(L"%s %s %s", comp_code, m_App->carecon_compCode, m_App->companyName);
			//AfxMessageBox(tst);
...
BOOL CregistryDlg::Update_MacAddr2(CString macAddr)
{
	...
	strQuery.Format(L"UPDATE %s.carecon_info SET macaddress2 = '%s' WHERE carecon_comp_code = '%s';", m_App->dbName.GetString(), macAddr.GetString(), m_App->carecon_compCode.GetString());
...
BOOL CregistryDlg::checkVersion(CString userVersion)
{
	...
	// get version info
	strQuery.Format(L"SELECT ver_info FROM %s.carecon_product where end_date IS NULL", m_App->dbName.GetString());
	...
	if (resultMsg != 0)
	{
		AfxMessageBox(L"Database에서 버전정보(carecon_product)를 가져오지 못했습니다");
		LogWrite(L"MySQL Error", L"Database에서 버전정보(carecon_product)를 가져오지 못했습니다");
		return true;
	}
	...
	{
		AfxMessageBox(L"Database에서 버전정보(carecon_product)를 가져오지 못했습니다");
		LogWrite(L"MySQL Error", L"Database Error : carecon_product mysql_store_result");
		return true;
	}
	...
	while (!token.IsEmpty() && idx < 3) {
		reDbVersion[idx++] = _ttoi(token);
		token = parDbVer.Tokenize(_T("."), pos);
	}

	for (int i = 0; i < 3; ++i) {
		if (reDbVersion[i] > reUserVersion[i]) {
			return false;	// DB 상위 버전 존재 → 업데이트 필요
		}
		if (reDbVersion[i] < reUserVersion[i]) {
			return true;	// 현재 버전이 더 높음 → 업데이트 불필요
		}
	}
...
BOOL CregistryDlg::LoadConfigure()
{
	...
	for (int i = 0; i < 4; i++) {
		m_App->buttonType[i] = L"";
		m_App->buttonName[i] = L"";
		m_App->buttonURL[i] = L"";
	}
	m_App->noticeNum = 0;
	for (int i = 0; i < 2; i++) {
		m_App->noticeType[i] = L"";
		m_App->noticeContent[i] = L"";
		m_App->noticeURL[i] = L"";
	}
	m_App->receiptNo = 0;
	for (int i = 0; i < 4; i++) {
		m_App->receiptIdx[i] = L"";
		m_App->receiptWriter[i] = L"";
		m_App->receiptSubject[i] = L"";
		m_App->receiptDate[i] = L"";
		m_App->receiptCodeName[i] = L"";
	}
	...
	// get 학교/기업명 정보 -------------------------------------------------------------------------
	strQuery.Format(L"SELECT comp_idx, home_page FROM %s.company_info WHERE del_yn = 'N' and comp_code = '%s'", m_App->dbName.GetString(), m_App->comp_code.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
	...
	// get 학교/기업 로고 이미지 -------------------------------------------------------------------------
	strQuery.Format(L"SELECT img_sname FROM %s.company_file WHERE comp_idx = '%s'", m_App->dbName.GetString(), m_App->comp_idx.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
	...
	// get 배너 이미지 -------------------------------------------------------------------------
	strQuery.Format(L"SELECT img_sname, link_url, comp_idx FROM %s.carecon_banner WHERE del_yn = 'N' and view_yn = 'Y' and (comp_mult = 'All' or concat(comp_mult, ',') LIKE '%%,%s,%%') ORDER BY sort LIMIT 3", m_App->dbName.GetString(), m_App->comp_idx.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
	...
	// Page 설정
	strQuery.Format(L"SELECT btn_type, btn_name, link_url FROM %s.carecon_button WHERE del_yn = 'N' and comp_idx = '%s' ORDER BY sort ASC", m_App->dbName.GetString(), m_App->comp_idx.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
	...
	// 공지사항
	strQuery.Format(L"SELECT import_type, content, link_url FROM %s.carecon_notice WHERE del_yn = 'N' and view_yn = 'Y' ORDER BY sort ASC LIMIT 2", m_App->dbName.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
	...
	strQuery.Format(L"SELECT ri.ri_idx, ri.subject, crs.code_name, ri.writer, ri.reg_date "
					 ...
					 "WHERE ri.del_yn = 'N' AND ri.comp_idx = '%s' ORDER BY ri.reg_date DESC LIMIT 4", m_App->dbName.GetString(), m_App->dbName.GetString(), m_App->comp_idx.GetString());
	strquery = W2A(strQuery);
	resultMsg = mysql_query(&conn, strquery);
...
UINT DownloadThread(LPVOID pParam)
{
	...
	{
		strURL.Format(L"https://new.bizstory.co.kr/data/company/%s/company/%s", m_App->comp_idx.GetString(), m_App->LogoImage.GetString());
		strFileName.Format(GetInstallDir() + L"%s", m_App->LogoImage.GetString());

		URLDownloadToFile(NULL, strURL, strFileName, 0, 0);
	}
	else if (pParams->strType == "banner")
	{
		if (pParams->number == 1)
		{
			strURL.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->bannerIDX[0].GetString(), m_App->bannerImg[0].GetString());
			strFileName.Format(GetInstallDir() + L"%s", m_App->bannerImg[0].GetString());
			URLDownloadToFile(NULL, strURL, strFileName, 0, 0);
		}
		else if (pParams->number == 2)
		{
			strURL1.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->bannerIDX[0].GetString(), m_App->bannerImg[0].GetString());
			strURL2.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->bannerIDX[1].GetString(), m_App->bannerImg[1].GetString());
			strFileName1.Format(GetInstallDir() + L"%s", m_App->bannerImg[0].GetString());
			strFileName2.Format(GetInstallDir() + L"%s", m_App->bannerImg[1].GetString());
			URLDownloadToFile(NULL, strURL1, strFileName1, 0, 0);
			URLDownloadToFile(NULL, strURL2, strFileName2, 0, 0);
		}
		else if (pParams->number == 3)
		{
			strURL1.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->comp_idx.GetString(), m_App->bannerImg[0].GetString());
			strURL2.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->comp_idx.GetString(), m_App->bannerImg[1].GetString());
			strURL3.Format(L"https://new.bizstory.co.kr/data/company/%s/banner/%s", m_App->comp_idx.GetString(), m_App->bannerImg[2].GetString());
			strFileName1.Format(GetInstallDir() + L"%s", m_App->bannerImg[0].GetString());
			strFileName2.Format(GetInstallDir() + L"%s", m_App->bannerImg[1].GetString());
			strFileName3.Format(GetInstallDir() + L"%s", m_App->bannerImg[2].GetString());
			URLDownloadToFile(NULL, strURL1, strFileName1, 0, 0);
			URLDownloadToFile(NULL, strURL2, strFileName2, 0, 0);
			URLDownloadToFile(NULL, strURL3, strFileName3, 0, 0);
		}
	}
