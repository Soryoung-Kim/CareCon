...
BOOL CCareConnDlg::OnInitDialog()
{
	...
	if (nRetCode <= 0)
	{
        // Count가 없거나 0 이하 → 재설치 여부 확인
        CString existingCareconCode = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"carecon_comp_code");

        if (!existingCareconCode.IsEmpty())
        {
            // 재설치: 기존 레지스트 값 유지, Count와 version만 현재 설치 버전으로 업데이트
            ...
            strVersion = m_App->strVersion;
            m_App->carecon_compCode = existingCareconCode;
            m_App->comp_code = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"comp_code");
            m_App->companyName = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"Company");
            m_App->manager = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"manager");
            m_App->place = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"place");
            ...
            else
            {
                ...
                {
                    int nResp = MessageBox(L"등록된 사용자 일치하지 않습니다. 다시 등록 해주세요.", _T("CareConn"), MB_ICONWARNING | MB_YESNO);
                    if (nResp == IDYES)
                    {
	...
	else
	{
        ...
        m_App->carecon_compCode = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"carecon_comp_code");
        m_App->comp_code = GetRegistryStrValue(REG_USER, L"Software\\METASTORY\\CareCon", L"comp_code");
...
void CCareConnDlg::callBizstory()
{
    ...
    url.Format(L"%s%s&macaddress=%s&agent_version=%s", baseUrl.GetString(), m_App->carecon_compCode.GetString(), m_App->activeMacCode.GetString(), m_App->strVersion.GetString());

    // Edge를 기본으로 열 때
    browserPath = GetRegistryStrValue(REG_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\msedge.exe", L"Path");
    if (browserPath != "")
    {
        pathURL.Format(L"%s\\msedge.exe", browserPath.GetString());
    ...
    else
    {
        ...
        if (browserPath != "")
        {
            pathURL.Format(L"%s\\chrome.exe", browserPath.GetString());
