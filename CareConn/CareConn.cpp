...
BOOL CCareConnApp::InitInstance()
{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////
		// 중복 실행 방지 코드
	HANDLE hMutexOneInstance = ::CreateMutex(NULL, TRUE, _T("Unique Name of CareCon v1.0"));
	...
	// product version (version.h 에서 관리)
	strVersion = APP_VERSION_WSTR;
	
	TCHAR dbHost[15], ui_company[255], ui_type[2];
	CString Path;
	Path.Format(_T("%s\\CareCon.ini"), L"C:\\Program Files (x86)\\METASTORY\\CareCon");
	::GetPrivateProfileString(_T("Info"), _T("UI_Type"), 0, ui_type, _countof(ui_type), Path);
	::GetPrivateProfileString(_T("Info"), _T("UI_Company"), 0, ui_company, _countof(ui_company), Path);
	::GetPrivateProfileString(_T("DB"), _T("HOST"), 0, dbHost, _countof(dbHost), Path);
	...
	CCareConnDlg* pDlg = new CCareConnDlg;
	m_pMainWnd = pDlg;
